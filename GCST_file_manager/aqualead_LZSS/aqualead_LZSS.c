...
int main_LZSS(char* pathIn, char* pathOut)
{
    ...
    if(0) // No input and/or output path specified
    {
        ...
        return EXIT_FAILURE;
    } else if(0) // output file not specified
    {
#ifdef WIN32
       char fname[_MAX_FNAME];
       char extension[_MAX_EXT];
       _splitpath(pathIn, NULL, NULL, fname, extension);
       ...
       sprintf(outfile_name, "out_%s%s", fname, extension);
        ...
        outfile_name = pathOut;
    }

    al_file = fopen(pathIn, "rb");
    if(!al_file)
    {
